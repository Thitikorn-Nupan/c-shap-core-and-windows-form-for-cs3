...
namespace UnderstandWindownsForms.sevices {
    ...
    internal class MobileService : ObjectRepo<Mobile> {
        ...
        private readonly string[] SQL_COMMAND =
        {
            ...
            "update mobiles set model = @model, brand = @brand, price = @price, amount = @amount where mbid = @mbid"
        };
        ...
        public bool createObject(Mobile obj) {
            connection.Open();
            // set up command for query
            command.Connection = connection;
        ...
        public bool editObject(Mobile obj, string mbid)
        {
            connection.Open();
            command.Connection = connection;
